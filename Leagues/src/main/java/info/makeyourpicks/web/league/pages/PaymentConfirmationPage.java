...
public class PaymentConfirmationPage extends LeagueMemberWebPage
{

	@SpringBean(name="leagueManager")
	protected LeagueManager leagueManager;
	
	public static final String WICKET_ID = "paymentConfirmationPage";
	...
	public PaymentConfirmationPage()
	{
		...
		String finalPaymentAmount =  payment.getFormatedAmount();
		...
		*/
		...
		HashMap nvp = ppf.ConfirmPayment ( token, payerId, finalPaymentAmount, ((ServletWebRequest)getRequest()).getHttpServletRequest().getServerName() );
		...
		if(strAck !=null && (strAck.equalsIgnoreCase("Success") || strAck.equalsIgnoreCase("SuccessWithWarning")))
		{
			...
			*/
			...
			String settleAmt		= (String)nvp.get("SETTLEAMT");  //' Amount deposited in your PayPal account after a currency conversion.
			if (settleAmt == null)
			{
				settleAmt="0";
			}
			String taxAmt			= nvp.get("TAXAMT").toString();  //' Tax charged on the transaction.
			String exchangeRate		= (String)nvp.get("EXCHANGERATE");  //' Exchange rate if a currency conversion occurred. Relevant only if your are billing in their non-primary currency. If the customer chooses to pay with a currency other than the non-primary currency, the conversion occurs in the customer’s account.
			if (exchangeRate==null)
			{
				exchangeRate="";
			}
			
			payment.setTransactionDate(orderTime);
			payment.setTransactionId(transactionId);
			payment.setPaymentType(paymentType);
			payment.setAmountPaid(Double.valueOf(amt));
			payment.setCurrencyCode(currencyCode);
			payment.setFeeAmount(Double.valueOf(feeAmt));
			payment.setSeatleAmount(Double.valueOf(settleAmt));
			payment.setTaxAmount(Double.valueOf(taxAmt));
			...
			*/
			...
			payment.setPendingReason(pendingReason);
			...
			*/
			...
			leagueManager.makePayment(payment);
			
			add(new PageLink("leagueHome",
//					LeagueWebConstants.homePageMap.get(getActiveLeague()
//							.getSeason().getLeagueType().getTypeOfLeague())));
//					((LeagueWebApplication)getApplication()).getLeagueHomePage(getActiveLeague().getSeason().getLeagueType().getTypeOfLeague())));
					getHomePageForLeague(getActiveLeague().getSeason().getLeagueType().getTypeOfLeague())));
//			add(new Label("amount", String.valueOf(payment.getAmountPaid())));
			add(new Label("transactionId", payment.getTransactionId()));
			add(new Label("orderNumber", String.valueOf(payment.getId())));
		}
		else
		{  
...
//	java.util.HashMap nvp = ConfirmPayment ( token, payerId, finalPaymentAmount, serverName );
//	String strAck = nvp.get("ACK").toString();
//	if(strAck !=null && !(strAck.equalsIgnoreCase("Success") || strAck.equalsIgnoreCase("SuccessWithWarning")))
//	{
//		/*
